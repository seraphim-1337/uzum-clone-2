...
function mountRelatedSlider() {
  ...
  slider.addEventListener('pointerdown', (event) => { dragging = true; startX = event.clientX; startScroll = slider.scrollLeft; slider.setPointerCapture(event.pointerId); slider.classList.add('related-products--dragging'); });
...
export function renderProductPage({ product, related, formatPrice }) {
  ...
  const recent = JSON.parse(localStorage.getItem('uzum-recent-products') || '[]').filter((id) => id !== product.id);
